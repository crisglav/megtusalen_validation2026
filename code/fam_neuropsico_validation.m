...
vars = struct( ...
    ...
    'BADS_rules_test', 'cog_res','DST_forward','DST_backward', ...
    'GDS_15', ...
    'LM_imm_units','LM_del_units','LM_imm_them','LM_del_them','MMSE','PFT_F','PFT_A','PFT_S', ...
    'ROCFB_copy', 'ROCFB_memory', 'SFT_animals', 'TMT_A_errors', 'TMT_A_time', 'TMT_B_errors', 'TMT_B_time', ...
    ...
    'Rey_Acopia', 'Rey_Amemoria', 'animales', 'TMTa_e','TMTa_t', 'TMTb_e', 'TMTb_t', ...
...
for ivar = 1:length(vars)
    ...
    for i = 1:n
        ...
        if contains(id,'FAMMA018')
            id = megtusalen.recording_id_orig{contains(megtusalen.(id_vars(1).megtusalen),'FAMMA018')};
            warning('%s participant is included also in UMEC.', id);
