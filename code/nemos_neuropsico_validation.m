...
nemos_neuro = readtable(nemos_neuro_excel ,'Sheet','Datos','VariableNamingRule','preserve');
% megtusalen = readtable(megtusalen_excel);

vars = struct( ...
    'megtusalen', ...
    {'age','sex','group', 'converter', 'conversion_time', 'edu_years', 'edu_level_nemos', 'occupation_nemos' ...
    'BADS_rules_test', 'BNT_spon', 'BNT_phon', 'clock_drawing', 'DST_forward','DST_backward', ...
    'FAQ', 'GDS_15', 'GloDS','imitation_gestures', ...
    'LM_imm_units','LM_del_units','LM_imm_them','LM_del_them',...
    'MMSE','PFT_F','PFT_A','PFT_S', 'PFT', ...
    'SFT_animals', 'SFT_fruits',  ...
    'TMT_A_errors', 'TMT_A_time', 'TMT_B_errors', 'TMT_B_time', ...
    }, ...
    'nemos', ...
    {'Edad','Sexo','Diagnostico', 'Conversores', 'Tiempo_conversion_en_meses', 'EduYears', 'Nivel_de_estudios', 'Ocupacion'...
    'Cambio_de_Reglas', 'BNT', 'BNT_Clave_Fon', 'RELOJ_Orden', 'DIGITOS_Directos', 'DIGITOS_Inversos', ...
    'FAQ', 'GDS_15', 'GDS', 'Imitacion_de_Posturas', ...
    'TEXTOS_Unid_Inmediatas', 'TEXTOS_Unid_Demoradas', 'TEXTOS_Tem_Rec_Inmed', 'TEXTOS_Tem_Rec_Demor', ...
    'MMSE', 'F', 'A', 'S', 'FAS_promedio_fonologico', ...
    'Animales', 'Frutas',  ...
    'TMT_A_Errores', 'TMT_A_Tiempo', 'TMT_B_Errores', 'TMT_B_Tiempo', ...
    });

% Unify ID Codes  in nemos
nemos_neuro.IDcorrected = sprintfc('NEMOS-%03d', nemos_neuro.ID_MEG);
